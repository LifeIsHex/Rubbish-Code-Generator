...
//Letter_Case:     0 - UpperCase     1 - LowerCase
function sbIntToHex(c: cardinal; digits: byte; letter_case: byte): string;
var
  a: cardinal;
  r: byte;
  temp: string;
...
begin
  ...
  begin
    ...
    if letter_case = 0 then
      temp := temp + hUpper[r + 1]
    else
      temp := temp + hLower[r + 1];
  end;
...
begin
  if Value < 0 then
    Result := '-$' + sbIntToHex(-Integer(Value), Digits, 1)
  else
    Result := '+$' + sbIntToHex(Integer(Value), Digits, 1);
end;
...
var
 ...
 //with the size of the read value
  ...
  begin
    if IsBadReadPtr(Address, 4) then //raise EDisAsmError.Create(err_not_enuff_code);
      Messagebox(0, 'R-DWord : [ Not enough code ]', 'Error', MB_ICONERROR);
  ...
  begin
    if IsBadReadPtr(Address, 2) then //raise EDisAsmError.Create(err_not_enuff_code);
      Messagebox(0, 'R-Word : [ Not enough code ]', 'Error', MB_ICONERROR);
  ...
  begin
    if IsBadReadPtr(Address, 1) then //raise EDisAsmError.Create(err_not_enuff_code);
      Messagebox(0, 'R-Byte : [ Not enough code ]', 'Error', MB_ICONERROR);
  ...
  begin
    ...
    Result := '$' + sbIntToHex(DWord(RefAddress), 4, 1);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  // Returns the description of the effective address in the ModRM byte.
   ...
   //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    // Save modification made by SS
      ...
      end; // End function ReadSib

   //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

  //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  // Start function GetEffectiveAddress
   ...
   // Call the OnRef proc.
    ...
    end; // End function GetEffectiveAddress
 ...
 // Start function Operand
  ...
  // Save the operand size using the DeffOperandSize and SubType
      ...
      'A':
        ...
        begin
          ...
          if AddressSize = 4 then
            ...
            Result := Result + '$' + sbIntToHex(ReadWord, 2, 1);

        end
        else
          // A direct address the isn't a pointer??
          //raise EDisAsmError.Create(err_invalid_operand);
          Messagebox(0, 'F-Operand anlayser : [ Invalid AddrMethod and OperandType combination ]', 'Error', MB_ICONERROR);

   // Reg field in ModRm specifies Control register.
            'C':
        if OperandType = 'd' then
        begin
        ...
        begin
          case OperandSize of
           ...
           //else raise EDisAsmError.Create(err_invalid_operand_size);
            ...
            Messagebox(0, 'F-Operand anlayser : [ Invalid OperandSize ]', 'Error', MB_ICONERROR);
          end;
   ...
   // Imidiate data
        ...
        begin
          ...
          for I := OperandSize downto 1 do
            Result := sbIntToHex(ReadByte, 2, 1) + Result;
          Result := '$' + Result;
          if Assigned(OnImmidiateData) then
   ...
   // Relative Offset Word or DWord
          'O':
        if AddressSize = 2 then
          Result := '%s' + Chr(OperandSize) + '[$' + sbIntToHex(ReadWord, 4, 1) + ']'
        else
        begin
   ...
   // Reg field in ModRM specifies a Segment register
            'S':
        case (ModRM and modrmReg) div 8 of
          0:
            Result := 'es';
          1:
            Result := 'cs';
 ...
 // End function Operand
  ...
  end; // End function GetInstruction
...
//010101010101010101010101010101010101010101010101010101010101010101010101010101
  ...
  // escape character
      'c':
        if SecondChar = '2' then
          Result := TwoByteOpcodes[char(ReadByte)]
        else if ModRm <= $BF then
...
begin
    ...
    case Result[I] of
      '%':
        begin
          case Result[I + 1] of
            's':
              if MustHaveSize then
                case Result[I + 2] of
                  #1:
                    Insert('byte ptr ', Result, I + 3);
                  #2:
                    Insert('word ptr ', Result, I + 3);
                  #4:
                    Insert('dword ptr ', Result, I + 3);
                  #6:
                    ;
                  #8:
                    Insert('qword ptr ', Result, I + 3);
                  #10:
                    Insert('tbyte ptr ', Result, I + 3);
                else
            ...
            'c':
              begin
                ...
                for J := Size - 1 downto 1 do
                  Insert(', $' + sbIntToHex(PByte(Address + J)^, 2, 1), Result, I + 3);
                Insert('DB  $' + sbIntToHex(PByte(Address)^, 2, 1), Result, I + 3);
              end;
          end;
          Delete(Result, I, 3);
        end;
 ...
 // Added by DaFixer - Fix the addresses to always be 8 chars in lenth
  ...
  begin
    ps2 := Pos(']', Result);
    if ps2 - ps1 < 10 then
      for k := ps2 - ps1 to 9 do
        Insert('0', Result, ps1 + 2);
  end;
